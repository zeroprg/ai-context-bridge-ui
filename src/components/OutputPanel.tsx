  ...
  const formatText = (text: string): JSX.Element[] => {
    ...
    const elements = text.split(complexRegex).map((fragment, index) => {
      ...
      if (isUserPrompt) {
        ...
        return <>
                  <p key={index} onDoubleClick={() => handleDoubleClick(fragment, index)} onMouseUp={() => handleTextSelection()}>
                  {fragment}&nbsp;&nbsp;
                  {isTextSelected ? (
                    <AudioPlayer endpoint={API_URLS.TTS("ECHO")} textToSpeechText={selectedText} start={true} />
                  ) : (
                    <AudioPlayer endpoint={API_URLS.TTS("ECHO")} textToSpeechText={fragment} start={false} />
                  )}
                  </p>
              </>;
